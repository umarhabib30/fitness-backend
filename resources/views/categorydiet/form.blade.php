...
<x-app-layout :assets="$assets ?? []">
    <div>
        <?php $id = $id ?? null;?>
        @if(isset($id))
            {{ html()->modelForm($data, 'PATCH', route('categorydiet.update', $id) )->attribute('enctype', 'multipart/form-data')->open() }}
        @else
            {{ html()->form('POST', route('categorydiet.store'))->attribute('enctype', 'multipart/form-data')->open() }} 
        @endif
        <div class="row">
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        ...
                        <div class="card-action">
                            <a href="{{ route('categorydiet.index') }} " class="btn btn-sm btn-primary" role="button">{{ __('message.back') }}</a>
                        </div>
                    </div>
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <div class="">
                                    <input class="form-control file-input" type="file" name="categorydiet_image" accept="image/*">
                                </div>
                            </div>
                            
                            @if( isset($id) && getMediaFileExit($data, 'categorydiet_image'))
                                <div class="col-md-2 mb-2 position-relative">
                                    <img id="categorydiet_image_preview" src="{{ getSingleMedia($data,'categorydiet_image') }}" alt="equipment-image" class="avatar-100 mt-1">
                                    <a class="text-danger remove-file" href="{{ route('remove.file', ['id' => $data->id, 'type' => 'categorydiet_image']) }}"
